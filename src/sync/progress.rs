...
pub async fn sync_progress_to_targets(
    ...
    // Live toggles: need position path and/or a played=true event with live_played on.
    ...
    for &(target_index, ref client_target) in target_clients {
        ...
        if let (Some(t_item_id), Some(t_user_id)) = (target_item_id, target_user_id) {
            ...
            let message = if played && send_played {
                format!("{} finished watching '{}'", user_name, item_title)
            } else {
...
/// Live-sync IsFavorite to all target servers (does not touch played/position).
